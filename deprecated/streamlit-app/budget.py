...
def save_budget(budget: Dict[str, float]) -> None:
    ...
    df = pd.DataFrame(
        [{"category": k, "monthly_budget": v} for k, v in budget.items()]
    )
    df.to_csv(BUDGET_PATH, index=False)


def budget_vs_actual(
    pnl_ledger: pd.DataFrame, budget: Dict[str, float]
) -> pd.DataFrame:
    ...
    result["pct_used"] = result.apply(
        lambda r: (r["avg_actual"] / r["monthly_budget"] * 100)
        if r["monthly_budget"] > 0
        else 0.0,
        axis=1,
    )
