...
def _classify_line(
    ...
    prev_dateless: Optional[Tuple[int, List[str]]],
...
def _pass1_classify_lines(
    ...
    txn_lines: Dict[int, List[str]] = {}
    prev_dateless: Optional[Tuple[int, List[str]]] = None
...
def _parse_transaction_row(texts: List[str], orphan_amounts: List[str]) -> Optional[Dict]:
...
def parse_pdf_words_to_df(
    file_bytes: bytes, filename: str = "", year_override: Optional[int] = None
